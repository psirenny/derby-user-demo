  ...
  app.fn('user', {
    ...
    photo: {
      edit: function (e, el) {
        var self = this
          , errors = e.at().get('errors') || []
          , origin = this.model.get('$config.origin')
          , blob = e.at().get('image.edited.blob')
          , data = new FormData()
          , options = {contentType: false, data: data, processData: false, type: 'POST'};

        e.at().set('event', 'submit');
        if (errors[0]) return;
        data.append('photo', blob);
        $.ajax(origin + '/user/photo/upload', options).done(function () {
          el.reset();
        });
      }
    },
    ...
    signin: function (e) {
      ...
      $.post(origin + '/user/signin', data)
        ...
        .fail(function () {
          data.submitted = true;
          self.history.push('/signin?' + qs.stringify(data));
        });
    },
    signout: function (e) {
