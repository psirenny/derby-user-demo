...
module.exports = function (app) {
  app.view.fn('github.repo', function (git) {
    return url.parse(git).path.split('/')[2].slice(0, -4);
  });

  app.view.fn('github.url', function (git) {
    return git.slice(0, -4);
  });

  app.view.fn('github.user', function (git) {
    return url.parse(git).path.split('/')[1];
  });
